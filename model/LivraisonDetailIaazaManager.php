<?php
class LivraisonDetailIaazaManager{
    //attributes
    private $_db;
    ...
    //CRUD operations
    public function add(LivraisonDetailIaaza $livraison){
        $query = $this->_db->prepare(
        'INSERT INTO t_livraison_detail_iaaza (designation, quantite, prixUnitaire, idLivraison, created, createdBy)
    ...
    public function update(LivraisonDetailIaaza $livraison){
        $query = $this->_db->prepare(
        'UPDATE t_livraison_detail_iaaza SET designation=:designation, quantite=:quantite, 
	...
	public function delete($id){
		$query = $this->_db->prepare('DELETE FROM t_livraison_detail_iaaza WHERE id=:id')
		or die(print_r($this->_db->errorInfo()));;
	...
	public function deleteLivraison($idLivraison){
		$query = $this->_db->prepare('DELETE FROM t_livraison_detail_iaaza WHERE idLivraison=:idLivraison')
		or die(print_r($this->_db->errorInfo()));;
	...
	public function getLivraisonsDetailByIdLivraison($idLivraison){
        ...
        $query = $this->_db->prepare('SELECT * FROM t_livraison_detail_iaaza WHERE idLivraison=:idLivraison
        ORDER BY id DESC');
        $query->bindValue(':idLivraison', $idLivraison);
        $query->execute();
        while($data = $query->fetch(PDO::FETCH_ASSOC)){
            $livraisonsDetail[] = new LivraisonDetailIaaza($data);
        }
        $query->closeCursor();
	...
	public function getLivraisonDetailNumber(){
        $query = $this->_db->query('SELECT COUNT(*) AS livraisonDetailNumber FROM t_livraison_detail_iaaza');
	...
	public function getNombreArticleLivraisonByIdLivraison($idLivraison){
		$query = $this->_db->prepare('SELECT COUNT(*) AS nombreArticle FROM t_livraison_detail_iaaza 
	...
	public function getTotalLivraison(){
		$query = $this->_db->query('SELECT SUM(prixUnitaire*quantite) AS totalLivraison FROM t_livraison_detail_iaaza')
		or die(print_r($this->_db->errorInfo()));
		$data = $query->fetch(PDO::FETCH_ASSOC);
		$query->closeCursor();
	...
	public function getTotalLivraisonByIdLivraison($idLivraison){
		$query = $this->_db->prepare('SELECT SUM(prixUnitaire*quantite) AS totalLivraison FROM t_livraison_detail_iaaza 
		WHERE idLivraison=:idLivraison')
		or die(print_r($this->_db->errorInfo()));;
		$query->bindValue(':idLivraison', $idLivraison);
		$query->execute();
